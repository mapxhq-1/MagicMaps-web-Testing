...
import { galaxyMotionConfigRef } from "../map/galaxyMotion";
...
export default function DeepSpaceBackground() {
  ...
  useEffect(() => {
    ...
    const palette = [new THREE.Color(0xffffff), new THREE.Color(0xaaddff), new THREE.Color(0xffddaa)];
    ...
    function animate(ts) {
      ...
      const dt = Math.min(Math.max((ts - lastFrameTs) / 1000, 0.001), 0.05);
      lastFrameTs = ts;
      time += 0.005;

      const cfg = galaxyMotionConfigRef.current;
      const glideBlend = 1 - Math.exp(-cfg.glideK * dt);

      const targetLng = mapViewState.lng;
      const targetLat = mapViewState.lat;
      const targetZoom = mapViewState.zoom;

      const smooth = smoothView.current;
      if (!smooth.ready) {
        smooth.lng = targetLng;
        smooth.lat = targetLat;
        smooth.zoom = targetZoom;
        smooth.ready = true;
      }

      smooth.lng += shortestLngDelta(smooth.lng, targetLng) * glideBlend;
      smooth.lat += (targetLat - smooth.lat) * glideBlend;
      smooth.zoom += (targetZoom - smooth.zoom) * glideBlend;

      let dLng = smooth.lng - prevMapState.current.lng;
      let dLat = smooth.lat - prevMapState.current.lat;
      const dZoom = smooth.zoom - prevMapState.current.zoom;

      if (dLng > 180) dLng -= 360;
      if (dLng < -180) dLng += 360;

      const motionGain = cfg.rotationGain;
      universeGroup.rotation.y += dLng * degToRad * motionGain;
      universeGroup.rotation.x += dLat * degToRad * motionGain;

      if (Math.abs(dZoom) > 0.00001) {
        universeZVelocity += dZoom * cfg.zoomImpulse;
      }
      universeZVelocity *= Math.exp(-cfg.coastDecayK * dt);
      universeGroup.position.z += universeZVelocity;

      const speedFactor = Math.abs(universeZVelocity);
      brightStars.material.size =
        15 + speedFactor * 0.5 * cfg.starMotionGain + Math.sin(time * 3) * 2;
      ...
      prevMapState.current.lng = smooth.lng;
      prevMapState.current.lat = smooth.lat;
      prevMapState.current.zoom = smooth.zoom;
    ...
    return () => {
      ...
      nebulaMat.dispose();
      trailGeo.dispose();
      smoothView.current.ready = false;
